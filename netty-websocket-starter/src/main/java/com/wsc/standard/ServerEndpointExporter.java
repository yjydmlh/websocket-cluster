...
import com.wsc.annotation.ServerEndpoint;
import com.wsc.exception.DeploymentException;
import com.wsc.pojo.PojoEndpointServer;
import com.wsc.pojo.PojoMethodMapping;
...
//            if (ClassUtils.isCglibProxyClass(endpointClass)) {
            if (endpointClass != null && endpointClass.getName().contains(CGLIB_CLASS_SEPARATOR)) {
    ...
    private void init() {
        for (Map.Entry<InetSocketAddress, WebsocketServer> entry : addressWebsocketServerMap.entrySet()) {
            ...
            try {
                ...
                logger.info(String.format("\033[34mNetty WebSocket started on port: %s with context path(s): %s .\033[0m", pojoEndpointServer.getPort(), stringJoiner));
